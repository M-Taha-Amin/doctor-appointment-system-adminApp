...
const AdminLogin = () => {
  ...
  const { admin, doctor } = useAppSelector(state => state.auth);
  ...
  const loginMutation = useMutation<any, Error, FormData>({
    mutationFn: async userData => {
      const res = await axiosClient(`${import.meta.env.VITE_SERVER_URL}/auth/admin-login`, {
        method: 'POST',
        data: userData,
      });
      return res.data;
    ...
    <div className="flex min-h-screen justify-center items-center">
      <div className="shadow-md ring ring-gray-200 rounded-lg p-8 w-sm">
        <h1 className="text-center text-2xl font-semibold text-gray-700">
          <span className="text-purple">Admin</span> Login
        </h1>
