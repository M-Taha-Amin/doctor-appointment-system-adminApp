...
const AdminHome = () => {
  ...
  const statsQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosClient(`${import.meta.env.VITE_SERVER_URL}/stats`);
          ...
          {appointmentsQuery.data?.slice(0, 6).map(appointment => (
            <div className="p-2 rounded-sm flex gap-2 text-sm items-center">
              ...
              {appointment.status === 'completed' ? (
                ...
                <button className="text-green-500 font-semibold ml-auto">
                  Paid Online
                </button>
              ) : (
                <div className="ml-auto">
                  {appointment.status === 'pending' && (
                    <button
                      className="bg-red-50 text-red-500 size-10 hover:scale-110 hover:shadow-sm rounded-full cursor-pointer"
